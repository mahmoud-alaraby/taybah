...
{{-- إذا لم يوجد شات بعد، اعرض صفحة اختيار الموظف --}}
@if(!isset($chat))
    <!-- No Chat Yet - Show Employee Selection -->
    <div class="p-6 bg-gray-50 min-h-screen">
        ...
        <div class="bg-white rounded-xl shadow-sm border p-6 mb-6">
            <div class="flex items-start justify-between">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 mb-2">لا يوجد شات بعد</h1>
                    <p class="text-gray-600">يجب اختيار موظف أولاً لبدء التواصل مع هذا العميل</p>
                </div>
    ...
    function assignEmployee(employeeId) {
        if (!confirm('هل تريد تعيين هذا الموظف للتواصل مع العميل؟')) return;
        
        fetch(`/admin/customer-communication/{{ $customer->id }}/assign-employee`, {
            method: 'POST',
            headers: {
...
@else
    {{-- الشات موجود - اعرض واجهة الشات الكاملة --}}
    <div class="flex h-[calc(100vh-200px)] bg-white shadow-xl rounded-2xl overflow-hidden">
        ...
        <div class="flex-1 flex flex-col">
            ...
            <!-- Chat Header -->
            <div class="flex items-center justify-between p-6 border-b bg-gradient-to-r from-red-600 to-red-700 text-white">
                <div class="flex items-center space-x-4 space-x-reverse">
                    <!-- Customer Avatar -->
                ...
                <div class="flex items-center space-x-3 space-x-reverse">
                    ...
                    <div class="flex items-center space-x-2 space-x-reverse">
                        ...
                        <button onclick="confirmClearChat()" 
                                class="bg-red-800 hover:bg-red-900 px-3 py-1 rounded-lg text-sm transition-colors" 
                                title="حذف الشات بالكامل">
                            <i class="fas fa-trash-alt ml-1"></i>
                            حذف الشات
                        </button>
                        
                        <!-- Clear Files Only -->
                        <button onclick="confirmClearFiles()" 
                                class="bg-orange-600 hover:bg-orange-700 px-3 py-1 rounded-lg text-sm transition-colors" 
                                title="حذف الملفات فقط">
                            <i class="fas fa-paperclip ml-1"></i>
                            حذف الملفات
                        </button>
                        
                        <a href="{{ route('admin.customer-communication.index') }}" 
                           class="text-red-100 hover:text-white p-2 transition-colors">
            ...
            <!-- File Preview Area -->
            <div id="filePreview" class="hidden border-t bg-gray-50 p-4 file-preview-enter">
                <div class="transition-all duration-300 border-2 border-dashed border-gray-300 rounded-lg p-4" id="filePreviewBox">
                    <div class="flex items-center gap-3" id="fileInfoContainer">
                        <div class="w-12 h-12 bg-red-100 rounded-lg flex items-center justify-center">
                            <i class="fas fa-file text-red-500 text-xl" id="fileIcon"></i>
                        </div>
                        <div class="flex-1">
                    ...
                    <div class="w-full h-1 bg-gray-200 rounded-full mt-3 overflow-hidden hidden" id="uploadProgress">
                        <div class="h-full bg-gradient-to-r from-red-500 to-red-600 rounded-full transition-all duration-300 w-0" id="uploadProgressBar"></div>
                    </div>
                    <div class="flex gap-2 mt-3">
                        <button onclick="sendSelectedFile()" 
                                class="bg-red-500 hover:bg-red-600 text-white px-3 py-1.5 rounded-lg text-sm transition-colors">
            ...
            <div class="border-t bg-white p-6">
                <form id="messageForm" class="flex items-end space-x-4 space-x-reverse">
                    ...
                    <div class="flex-1 relative">
                        <input type="text" id="messageInput" placeholder="اكتب رسالتك للموظف بخصوص هذا العميل..." 
                               class="w-full px-6 py-4 rounded-2xl border-gray-300 focus:border-red-500 focus:ring-2 focus:ring-red-500 focus:ring-opacity-20 pr-16 text-lg">
                        
                        <!-- Send Button -->
                        <button type="submit" 
                                class="absolute left-3 top-1/2 transform -translate-y-1/2 w-10 h-10 bg-gradient-to-r from-red-500 to-red-600 hover:from-red-600 hover:to-red-700 rounded-full flex items-center justify-center transition-all shadow-lg hover:shadow-xl">
    ...
    function loadNewMessages() {
        ...
        fetch(`/admin/customer-communication/${chatId}/new-messages?last_message_id=${lastMessageId}`)
            .then(response => {
    ...
    // باقي الكود للشات - إرسال الرسائل، الملفات، الصوت، الخ...
    ...
    function sendTextMessage() {
        ...
        fetch(`/admin/customer-communication/${chatId}/send`, {
            method: 'POST',
            body: formData
        })
        .then(response => response.json())
    ...
    function generateMessageHTML(message) {
        let contentHTML = `<p class="break-words leading-relaxed">${message.content}</p>`;
        
        const deleteButton = message.sender_type === 'admin' && message.can_delete ? 
            `<button onclick="deleteMessage(${message.id})" class="text-red-400 hover:text-red-300 transition-colors">
                <i class="fas fa-trash text-xs"></i>
            </button>` : '';
    ...
    function confirmClearFiles() {
        ...
        .then(data => {
            if (data.success) {
                alert(`تم حذف ${data.deleted_count} ملف`);
                location.reload();
            }
        });
    }
    ...
    function confirmClearChat() {
        if (!confirm('هل أنت متأكد من حذف الشات بالكامل؟')) return;
        if (!confirm('تأكيد أخير!')) return;
        
        fetch(`/admin/customer-communication/${chatId}/clear-entire-chat`, {
            method: 'DELETE',
            headers: {
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                'Content-Type': 'application/json'
        ...
        .then(data => {
            if (data.success) {
                ...
                window.location.href = '/admin/customer-communication';
            }
        });
    }
...
function sendTextMessage() {
    ...
    .then(data => {
        if (data.success) {
            ...
            alert(`تم حذف ${data.deleted_count} ملف وتوفير ${data.freed_space} من مساحة التخزين`);
            location.reload();
        } else {
...
// تأكيد حذف الشات بالكامل
function confirmClearChat() {
    if (!confirm('هل أنت متأكد من حذف الشات بالكامل؟\nسيتم حذف جميع الرسائل والملفات ولن يمكن استرجاعها.')) return;
    if (!confirm('تأكيد أخير: سيتم حذف الشات نهائياً!')) return;
...
function showNewMessageNotification() {
    if ("Notification" in window && Notification.permission === "granted") {
        new Notification("رسالة جديدة من الموظف", {
...
@endpush
@endsection('حدث خطأ في إرسال الرسالة');
        }
    })
    .catch(error => {
        console.error('Error:', error);
...
function showFilePreview(file) {
    ...
    previewBox.classList.add('border-red-300', 'bg-red-50');
...
function sendSelectedFile() {
    ...
    previewBox.classList.remove('border-red-300', 'bg-red-50');
...
// تسجيل صوتي (نفس المنطق مع تعديل الألوان للأحمر)
...
// باقي الوظائف للشات (addNewMessageToChat, deleteMessage, scrollToBottom, etc.)
function addNewMessageToChat(message) {
    const messagesList = document.getElementById('messagesList');
...
// تأكيد حذف الملفات فقط
function confirmClearFiles() {
    if (!confirm('هل أنت متأكد من حذف جميع الملفات في هذا الشات؟\nسيتم حذف الملفات والرسائل الصوتية فقط وليس الرسائل النصية.')) return;
