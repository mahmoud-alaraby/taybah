            ...
            <tbody>
                @foreach($attendances as $attendance)
                    <tr>
                        ...
                        <td>{{ $attendance->check_out_time ? $attendance->check_out_time->format('H:i') : 'لم ينصرف' }}</td>
                        <td>{{ number_format($attendance->total_hours, 1) }}</td>
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 20px;">
            ...
            <div>
                ...
                <div style="font-size: 24px; font-weight: bold; color: #6f42c1;">
                    {{ number_format($monthlyStats['average_daily_hours'], 1) }} ساعة
                </div>
                <div style="background: #e9ecef; height: 10px; border-radius: 5px; overflow: hidden;">
                    <div style="background: #6f42c1; height: 100%; width: {{ min(100, ($monthlyStats['average_daily_hours'] / 8) * 100) }}%;"></div>
                </div>
            </div>
        </div>
