...
@section('page-subtitle', 'إدارة تكاليف التصوير والمقبوضات والمدفوعات ومتابعة التارجت الشهري لجميع الموظفين')
...
<div class="space-y-6">
        ...
        <div class="bg-gradient-to-r from-blue-500 to-blue-600 p-4 text-white">
            <div class="flex items-center justify-between mb-4">
                <h2 class="text-lg font-bold flex items-center">
                    ...
                    تحديد التارجت الشهري للتصوير - {{ $months[$currentMonth] }} {{ $currentYear }}
                </h2>
            ...
            <form action="{{ route('admin.photography-costs.target') }}" method="POST" class="grid grid-cols-5 gap-3">
                @csrf
                <div>
    ...
    <div class="bg-white shadow rounded-lg p-4">
        <form method="GET" class="grid grid-cols-7 gap-3 items-end">
            ...
            <div>
                <a href="{{ route('admin.photography-costs.print', ['year' => $currentYear, 'month' => $currentMonth, 'search' => $search, 'employee_id' => $employeeId]) }}" 
                   target="_blank"
    ...
    <div class="grid grid-cols-2 gap-6">
        ...
        <div class="bg-white shadow rounded-lg">
            ...
            <div id="receiptForm" class="hidden bg-green-50 border-b border-green-200 p-4">
                <form action="{{ route('admin.photography-costs.receipt.store') }}" method="POST">
                    @csrf
                    <div class="space-y-3">
                        <div>
        ...
        <div class="bg-white shadow rounded-lg">
            ...
            <div id="paymentForm" class="hidden bg-red-50 border-b border-red-200 p-4">
                <form action="{{ route('admin.photography-costs.payment.store') }}" method="POST">
                    @csrf
                    <div class="space-y-3">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">البيان</label>
...
function deleteItem(type, id) {
    ...
    confirmDelete(title, text).then((result) => {
        if (result.isConfirmed) {
            ...
            form.method = 'POST';
            form.action = `/admin/photography-costs/${type}/${id}`;
