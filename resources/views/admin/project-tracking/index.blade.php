...
<div class="grid grid-cols-1 md:grid-cols-4 gap-4">
    ...
    <div class="bg-white p-6 rounded-lg shadow-lg border-l-4 border-blue-500 attendance-card">
        <div class="flex items-center justify-between mb-4">
            <div>
                <p class="text-base font-semibold text-blue-900 flex items-center space-x-2 rtl:space-x-reverse">
                    <svg class="w-6 h-6 text-blue-700 ml-2" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
        ...
        @if($todayAttendance)
            <div class="mb-4 p-3 bg-blue-50 rounded-lg">
                <div class="text-sm text-blue-700">
                    ...
                    @if($todayAttendance->check_out_time)
                        ...
                        <p class="flex items-center justify-between mt-1">
                            <span>الإجمالي:</span>
                            <span class="font-semibold">{{ formatHoursToHoursMinutes($todayAttendance->total_hours) }}</span>
                        </p>
                    @endif
            ...
            <div class="flex space-x-2 rtl:space-x-reverse">
                @if($todayAttendance->check_out_time)
                    <span class="px-4 py-2 bg-gray-100 text-gray-800 rounded-full text-sm font-semibold flex-1 text-center">
                        تم الانصراف
                    </span>
                @else
                    <button @click="checkOut()" 
                            class="px-4 py-2 bg-red-600 text-white rounded-lg text-sm font-semibold hover:bg-red-700 transition-colors duration-200 flex-1">
    ...
    <!-- العداد النشط للأدمن -->
    <div class="bg-white p-6 rounded-lg shadow-lg border-l-4 border-yellow-500 {{ $activeTimer ? 'active-timer' : '' }}">
        ...
        @if($activeTimer)
            <div class="text-center">
                <div class="text-2xl font-bold text-yellow-900 font-mono mb-2" x-text="activeTimerDisplay">00:00</div>
                ...
                <div class="flex space-x-2 rtl:space-x-reverse">
                    <button @click="stopMyTimer()" class="flex-1 px-3 py-2 bg-red-600 text-white rounded-lg text-sm font-semibold hover:bg-red-700 transition-colors duration-200">
                        <i class="fas fa-stop"></i> إيقاف
                    </button>
                    <button @click="pauseMyTimer()" class="flex-1 px-3 py-2 bg-yellow-600 text-white rounded-lg text-sm font-semibold hover:bg-yellow-700 transition-colors duration-200">
        ...
        @else
            <div class="text-center">
                ...
                <button @click="showStartModal = true" class="w-full px-4 py-3 bg-yellow-600 text-white rounded-lg text-sm font-semibold hover:bg-yellow-700 transition-colors duration-200">
    ...
    <div class="bg-white p-6 rounded-lg shadow-lg border-l-4 border-green-500">
        ...
        <div class="text-center">
            <div class="text-2xl font-bold text-green-900 mb-2">{{ formatHoursToHoursMinutes($todayStats['total_hours']) }}</div>
    ...
    <div class="bg-white p-6 rounded-lg shadow-lg border-l-4 border-purple-500">
        ...
        <div class="text-center">
            ...
            <div class="text-sm text-purple-700 font-medium">{{ $todayStats['projects_worked'] }} مشاريع اليوم</div>
        </div>
    </div>
</div>
                    ...
                    @foreach($activeProjects as $project)
                        <div class="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow duration-200">
                            <div class="flex justify-between items-start mb-3">
                                <div class="flex-1">
                                    <h4 class="font-semibold text-gray-900 text-base flex items-center space-x-2 rtl:space-x-reverse">
                            ...
                            @if($project->tasks->count() > 0)
                                <div class="space-y-2 mt-3">
                                    @foreach($project->tasks->take(3) as $task)
                                        <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg task-item">
                                            <div class="flex-1">
                                                <p class="text-sm font-medium text-gray-900 flex items-center space-x-2 rtl:space-x-reverse">
                                                    <i class="fas fa-tasks text-yellow-600 ml-2"></i>
                                                    <span>{{ $task->name }}</span>
                                                </p>
                                                <div class="flex items-center space-x-4 rtl:space-x-reverse text-xs text-gray-600 mt-1">
        ...
        <div class="p-4">
            <div x-show="todayEntries.length > 0" class="space-y-3">
                <template x-for="entry in todayEntries" :key="entry.id">
                    <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg border-r-4 border-green-400 hover:shadow-md transition-shadow duration-200">
                        <div class="flex-1">
                            <p class="text-sm font-semibold text-green-800 mb-1 flex items-center space-x-2 rtl:space-x-reverse">
            ...
            <div x-show="todayEntries.length === 0" class="text-center py-16 text-green-400">
                <svg class="mx-auto w-20 h-20 mb-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
...
<!-- مودال بدء العداد -->
<div x-show="showStartModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50 flex items-center justify-center" 
    ...
    <div class="bg-white p-6 border w-96 shadow-xl rounded-xl" 
         x-transition:enter="ease-out duration-300"
...
<!-- مودال إنشاء مشروع -->
<div x-show="showCreateModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50 flex items-center justify-center" 
     x-transition:enter="ease-out duration-300"
     x-transition:enter-start="opacity-0"
     x-transition:enter-end="opacity-100"
     x-transition:leave="ease-in duration-200"
...
<!-- مودال إضافة مهمة -->
<div x-show="showTaskModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50 flex items-center justify-center" 
     x-transition:enter="ease-out duration-300"
     x-transition:enter-start="opacity-0"
     x-transition:enter-end="opacity-100"
     x-transition:leave="ease-in duration-200"
