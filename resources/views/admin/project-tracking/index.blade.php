...
@section('content')
    <div class="min-h-screen bg-gradient-to-br from-slate-100 via-blue-50 to-indigo-100" x-data="adminProjectTracking()"
        x-init="init()">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
            {{-- Attendance Card --}}
            @include('admin.project-tracking.components.attendance-card')

            {{-- Timer Card --}}
            @include('admin.project-tracking.components.timer-card')
        </div>

        {{-- Stats Cards --}}
        <div class="mb-8">
            @include('admin.project-tracking.components.stats-cards')
        </div>
        ...
        <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
            ...
            <div class="xl:col-span-2 space-y-6">
                {{-- Active Projects --}}
                @include('admin.project-tracking.components.projects-list')
            </div>

            {{-- Today's Work Log --}}
            <div class="xl:col-span-1">
                @include('admin.project-tracking.components.today-entries')
            </div>
        </div>

        {{-- Modals --}}
        @include('admin.project-tracking.components.modals')
        @include('admin.project-tracking.components.project-task-modals')
    </div>
                ...
                return {
                    ...
                    async pauseTimer() {
                        try {
                            ...
                            if (data.success) {
                                // إيقاف العداد المرئي أولاً
                                if (this.timerInterval) {
                                    clearInterval(this.timerInterval);
                                    this.timerInterval = null;
                                }

                                // عرض الرسالة وبعدها ريفريش فوراً
                                await this.showAlert('تم إيقاف العداد مؤقتاً', 'success');
                                location.reload(); // ريفريش مباشر بعد الرسالة
                            } else {
                                this.showAlert(data.message, 'error');
                            }
                        } catch (error) {
                    ...
                    // استبدل دالة resumeTimer بهذه:
                    async resumeTimer() {
                        try {
                            const response = await fetch('{{ route('admin.project-tracking.resume-timer') }}', {
                                method: 'POST',
                                headers: {
                                    'Content-Type': 'application/json',
                                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute(
                    ...
                    async restartTimer(timerId) {
                        try {
                            const response = await fetch('{{ route('admin.project-tracking.restart-timer') }}', {
                                method: 'POST',
                                headers: {
                                    'Content-Type': 'application/json',
                                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute(
                    ...
                    async editTimer() {
                        ...
                        try {
                            const response = await fetch('{{ route('admin.project-tracking.edit-timer') }}', {
                                method: 'POST',
                                headers: {
                                    'Content-Type': 'application/json',
                                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute(
                            ...
                            if (data.success) {
                                ...
                                this.loadTodayEntries(); // Refresh entries
                            } else {
                                this.showAlert(data.message, 'error');
                            }
                        } catch (error) {
                    ...
                    async showTimerDetails(timerId) {
                        try {
                            const response = await fetch('{{ route('admin.project-tracking.timer-details') }}', {
                                method: 'POST',
                                headers: {
                                    'Content-Type': 'application/json',
                                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute(
