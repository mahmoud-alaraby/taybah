...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
    {{-- عنوان الصفحة --}}
    <h1 class="text-2xl font-bold mb-6">تعديل مهمة</h1>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        ...
        {{-- بداية الفورم --}}
        <form method="POST" action="{{ route('admin.designer-task-accounts.update', $account->id) }}" class="p-6 space-y-8">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="date" name="task_date" id="task_date" value="{{ old('task_date', $account->task_date) }}" required
                ...
                <div>
                    ...
                    <select name="designer_id" id="designer_id" required
                        ...
                        @foreach($designers as $designer)
                            <option value="{{ $designer->id }}" @selected(old('designer_id', $account->designer_id) == $designer->id)>{{ $designer->name }}</option>
                        @endforeach
                    </select>
            ...
            <div id="tasks-wrapper" class="space-y-6">
                ...
                @php
                    $oldTasks = old('tasks', $tasks);
                @endphp

                @foreach($oldTasks as $index => $task)
                    <div class="border border-gray-200 rounded-xl p-6 relative bg-gray-50 shadow-sm task-item">
                        <div class="mb-4">
            ...
            <div class="flex justify-end gap-4 pt-8 border-t border-gray-200">
                ...
                <button type="submit" 
                        class="inline-flex items-center justify-center px-8 py-3 bg-gradient-to-r from-red-600 to-red-700 hover:from-green-700 hover:to-green-800 text-white rounded-lg shadow-sm transition-all duration-200 gap-2">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // عدد المهام الحالية
    let currentTaskCount = tasksWrapper.querySelectorAll('.task-item').length || {{ count($oldTasks) ?? 1 }};
