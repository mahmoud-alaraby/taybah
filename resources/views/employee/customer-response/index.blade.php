...
<div class="max-w-6xl mx-auto">
    <div class="flex flex-col md:flex-row justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-800 flex items-center gap-2">
                <svg class="w-8 h-8 text-indigo-600" fill="none"></svg>
                قاموس الردود الجاهزة
            </h1>
            <p class="text-gray-500 mt-1">نصوص جاهزة للرد على العملاء حسب التصنيف</p>
        </div>
        <div>
            <a href="{{ route('employee.customer-response.create') }}"
    ...
    <form method="get" class="flex flex-wrap items-center gap-3 mb-8">
        ...
        <button type="submit" class="bg-gray-800 text-white px-8 py-2 rounded-lg"><i class="fas fa-search ml-1"></i>بحث</button>
    </form>

    <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6 mt-16">
            ...
            <div class="relative bg-white shadow p-5 flex flex-col border border-gray-200 rounded-2xl min-h-[260px] group transition duration-200 hover:-translate-y-1 hover:shadow-lg"
                ...
                <div class="flex items-start gap-4 mb-2">
                    ...
                    <span class="block flex-1 text-md font-bold text-gray-800 truncate">
                        ...
                        <a href="{{ route('employee.customer-response.edit', $res) }}" title="تعديل"
                        ...
                        <form action="{{ route('employee.customer-response.destroy',$res) }}" method="POST" onsubmit="return confirm('تأكيد الحذف؟')" class="inline">
                            @csrf @method('DELETE')
