...
@if(auth('employee')->user()->hasPermission('print_costs'))
    <a href="{{ route('employee.print-costs.index') }}" 
       class="group flex items-center px-3 py-2 text-sm font-medium rounded-md transition-colors prevent-flash {{ request()->routeIs('employee.print-costs.*') ? 'bg-red-600 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white' }}">
        <i class="fas fa-print ml-2 text-sm"></i>
        <span class="flex-1">تكاليف الطباعة</span>
    </a>
@endif
              @if(auth('employee')->user()->hasPermission('designers_account'))
                    <a href="{{ route('employee.designer-task-accounts.index') }}" 
