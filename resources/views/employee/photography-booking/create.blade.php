{{-- resources/views/employee/photography-booking/create.blade.php --}}
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
            ...
            <p class="text-slate-600 mt-1 text-lg">قم بملء جميع البيانات المطلوبة لإنشاء حجز جديد</p>
        </div>
        <div class="flex flex-col sm:flex-row gap-3">
            <a href="{{ route('employee.photography-booking.index') }}" 
               class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-gray-500 to-gray-600 hover:from-gray-600 hover:to-gray-700 text-white rounded-lg shadow-sm transition-all duration-200">
    ...
    @if(session('success'))
        <div class="mb-6 bg-green-50 border border-green-200 rounded-xl p-4">
            <div class="flex items-center">
                ...
                <div>
                    <h4 class="text-green-800 font-semibold">{{ session('success') }}</h4>
                </div>
            </div>
        </div>
    @endif
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        ...
        <form action="{{ route('employee.photography-booking.store') }}" method="POST" class="p-6">
            @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    <div>
                        ...
                        <select name="status" required
                            ...
                            <option value="in_progress" {{ old('status') == 'in_progress' ? 'selected' : '' }}>جاري العمل</option>
                    ...
                    <div>
                        ...
                        <input type="text" name="client_name" required
                               ...
                               value="{{ old('client_name') }}" placeholder="أدخل اسم العميل">
                        @error('client_name')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Date validation
    const firstSession = document.querySelector('input[name="first_session"]');
    const lastSession = document.querySelector('input[name="last_session"]');
    
    function validateDates() {
        if (firstSession.value && lastSession.value && firstSession.value > lastSession.value) {
            lastSession.classList.add('border-red-500');
            alert('تاريخ آخر سيشن يجب أن يكون بعد تاريخ أول سيشن');
        } else {
            firstSession.classList.remove('border-red-500');
            lastSession.classList.remove('border-red-500');
        }
    }
    
    if (firstSession && lastSession) {
        firstSession.addEventListener('change', validateDates);
        lastSession.addEventListener('change', validateDates);
    }
