<?php
// resources/views/employee/print-booking/show.blade.php
?>
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-white px-6 py-4 rounded-t-xl">
                <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between space-y-3 sm:space-y-0">
                    ...
                    <div class="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2 sm:space-x-reverse">
                        @if(auth('employee')->user()->id == $printBooking->created_by_employee || auth('employee')->user()->id == $printBooking->assigned_person_id)
                        <a href="{{ route('employee.print-booking.edit', $printBooking) }}" class="inline-flex items-center px-4 py-2 bg-red-600 text-white font-medium rounded-lg hover:bg-red-700 transition-colors">
                        ...
                        @endif
                        <a href="{{ route('employee.print-booking.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-600 text-white font-medium rounded-lg hover:bg-gray-800 transition-colors">
                            <svg class="w-4 h-4 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                    <label class="text-sm opacity-80 block mb-2">اسم العميل</label>
                    <p class="text-lg font-semibold bg-gray-50 rounded-lg py-2 px-3">
                        {{ $printBooking->client_name }}
                    </p>
                </div>
                <div>
                    <label class="text-sm opacity-80 block mb-2">رقم الجوال</label>
                    <p class="text-lg font-semibold bg-gray-50 rounded-lg py-2 px-3">
                        {{ $printBooking->client_phone ?: 'غير محدد' }}
                    </p>
                </div>
                <div>
                    <label class="text-sm opacity-80 block mb-2">الحالة</label>
                    <span class="px-3 py-1 rounded-full text-sm font-medium
                        @if($printBooking->status == 'in_progress') bg-yellow-100 text-yellow-800
                        @elseif($printBooking->status == 'completed') bg-green-100 text-green-800
                        @else bg-red-100 text-red-800
                        @endif">
                        {{ $printBooking->status_label }}
                    </span>
                </div>
                <div>
                    <label class="text-sm opacity-80 block mb-2">المسؤول عن التنفيذ</label>
                    <p class="text-lg font-semibold bg-gray-50 rounded-lg py-2 px-3">
                        {{ $printBooking->assignedPerson->name ?? 'غير محدد' }}
                    </p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-xl border border-gray-200 p-8">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                @if(auth('employee')->user()->id == $printBooking->created_by_employee || auth('employee')->user()->id == $printBooking->assigned_person_id)
                <a href="{{ route('employee.print-booking.edit', $printBooking) }}" 
                ...
                @if(auth('employee')->user()->id == $printBooking->created_by_employee)
                <form action="{{ route('employee.print-booking.destroy', $printBooking) }}" method="POST" class="inline">
                    @csrf
                    @method('DELETE')
                    <button type="submit" 
