<?php
// resources/views/employee/print-booking/edit.blade.php
?>
@extends('employee.layouts.app')

@section('title', 'تعديل حجز الطباعة')
...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div class="bg-gradient-to-r from-red-600 to-red-700 px-6 py-4">
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between space-y-4 sm:space-y-0">
                <h2 class="text-lg font-semibold text-white flex items-center">
                    ...
                    تعديل بيانات حجز الطباعة
                </h2>
                <div class="flex flex-col sm:flex-row gap-3">
                    <a href="{{ route('employee.print-booking.show', $printBooking) }}" 
        ...
        <form action="{{ route('employee.print-booking.update', $printBooking) }}" method="POST" class="p-6">
            @csrf
            @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <div>
                        ...
                        <select name="status" required
                            ...
                            <option value="in_progress" {{ old('status', $printBooking->status) == 'in_progress' ? 'selected' : '' }}>جاري العمل</option>
                            <option value="completed" {{ old('status', $printBooking->status) == 'completed' ? 'selected' : '' }}>تم الانتهاء</option>
                            <option value="bad_debt" {{ old('status', $printBooking->status) == 'bad_debt' ? 'selected' : '' }}>ديون معدومة</option>
                        </select>
                        @error('status')
                    ...
                    <div>
                        ...
                        <input type="text" name="client_name" required
                               ...
                               value="{{ old('client_name', $printBooking->client_name) }}" placeholder="الاسم الكامل للعميل">
                        @error('client_name')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="tel" name="client_phone"
                               ...
                               value="{{ old('client_phone', $printBooking->client_phone) }}" placeholder="05xxxxxxxx">
                        @error('client_phone')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <textarea name="work_description" rows="4" required
                                  ...
                                  placeholder="اكتب وصفاً تفصيلياً للعمل المطلوب">{{ old('work_description', $printBooking->work_description) }}</textarea>
                    ...
                    <div>
                        ...
                        <textarea name="agreement" rows="4"
                                  ...
                                  placeholder="تفاصيل الاتفاق مع العميل">{{ old('agreement', $printBooking->agreement) }}</textarea>
                        @error('agreement')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="lg:col-span-2">
                        ...
                        <textarea name="work_notes" rows="3"
                                  ...
                                  placeholder="أي ملاحظات إضافية">{{ old('work_notes', $printBooking->work_notes) }}</textarea>
                        @error('work_notes')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <div>
                        ...
                        <input type="date" name="first_order"
                               ...
                               value="{{ old('first_order', $printBooking->first_order ? $printBooking->first_order->format('Y-m-d') : '') }}">
                        @error('first_order')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="date" name="last_order"
                               ...
                               value="{{ old('last_order', $printBooking->last_order ? $printBooking->last_order->format('Y-m-d') : '') }}">
                        @error('last_order')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" name="orders_count" min="1"
                               ...
                               value="{{ old('orders_count', $printBooking->orders_count ?? 1) }}">
                        @error('orders_count')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="date" name="initial_delivery"
                               ...
                               value="{{ old('initial_delivery', $printBooking->initial_delivery ? $printBooking->initial_delivery->format('Y-m-d') : '') }}">
                    ...
                    <div>
                        ...
                        <input type="date" name="final_delivery"
                               ...
                               value="{{ old('final_delivery', $printBooking->final_delivery ? $printBooking->final_delivery->format('Y-m-d') : '') }}">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    ...
                    <div>
                        ...
                        <input type="date" name="booking_date"
                               ...
                               value="{{ old('booking_date', $printBooking->booking_date ? $printBooking->booking_date->format('Y-m-d') : '') }}">
                        @error('booking_date')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="time" name="booking_time"
                               ...
                               value="{{ old('booking_time', $printBooking->booking_time ? $printBooking->booking_time->format('H:i') : '') }}">
                        @error('booking_time')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" name="duration_hours" min="1" max="24"
                               ...
                               value="{{ old('duration_hours', $printBooking->duration_hours ?? 1) }}">
                    ...
                    <div>
                        ...
                        <input type="text" name="location"
                               ...
                               value="{{ old('location', $printBooking->location) }}" placeholder="مكان تنفيذ الحجز">
                        @error('location')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="md:col-span-2 lg:col-span-4">
                        ...
                        <textarea name="notes" rows="3"
                                  ...
                                  placeholder="أي ملاحظات أو تعليمات خاصة">{{ old('notes', $printBooking->notes) }}</textarea>
                        @error('notes')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    const firstOrderInput = document.querySelector('input[name="first_order"]');
    const lastOrderInput = document.querySelector('input[name="last_order"]');
    const initialDeliveryInput = document.querySelector('input[name="initial_delivery"]');
    const finalDeliveryInput = document.querySelector('input[name="final_delivery"]');
    
    // تحديث آخر أوردر عند تغيير أول أوردر
    if (firstOrderInput && lastOrderInput) {
        firstOrderInput.addEventListener('change', function() {
            if (this.value && !lastOrderInput.value) {
                lastOrderInput.value = this.value;
            }
        });
    }
    
    // تحديث التسليم النهائي عند تغيير التسليم الأولي
    if (initialDeliveryInput && finalDeliveryInput) {
        initialDeliveryInput.addEventListener('change', function() {
            if (this.value && !finalDeliveryInput.value) {
    ...
    document.querySelector('form').addEventListener('submit', function(e) {
        const firstOrder = firstOrderInput ? firstOrderInput.value : '';
        const lastOrder = lastOrderInput ? lastOrderInput.value : '';
        const initialDelivery = initialDeliveryInput ? initialDeliveryInput.value : '';
        const finalDelivery = finalDeliveryInput ? finalDeliveryInput.value : '';
        
        // التحقق من أن آخر أوردر بعد أول أوردر
        if (firstOrder && lastOrder && new Date(lastOrder) < new Date(firstOrder)) {
            e.preventDefault();
            alert('تاريخ آخر أوردر يجب أن يكون بعد تاريخ أول أوردر');
            return false;
        }
        
        // التحقق من أن التسليم النهائي بعد التسليم الأولي
        if (initialDelivery && finalDelivery && new Date(finalDelivery) < new Date(initialDelivery)) {
            e.preventDefault();
            alert('تاريخ التسليم النهائي يجب أن يكون بعد تاريخ التسليم الأولي');
            return false;
        }
    });
});
